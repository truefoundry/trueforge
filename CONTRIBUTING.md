...
- `PUBLIC_BASE_URL` — public origin for MCP OAuth / OIDC callbacks. Used as-is for `pnpm standalone:dev` (standalone + `NODE_ENV=development`); required when `STANDALONE=false`. Other standalone modes fall back to `http://localhost:$PORT`.
