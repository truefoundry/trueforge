...
import { TrueforgeUI, type ChatLayout } from './TrueforgeUI.js';

function mockServer(catalog?: CatalogServer): AgentUIServer {
  return {
    createSession: vi.fn(),
    listSessions: vi.fn(),
    getSession: vi.fn(),
...
const stubCatalog = {} as CatalogServer;
...
describe('SidebarLayout', () => {
  ...
  it('toggles theme from the footer and shows settings only when catalog is provided', () => {
    ...
    const themeBtn = screen.getAllByRole('button', { name: /Switch to (light|dark) theme/ })[0]!;
