...
describe("SandboxSettings", () => {
  it("autofills create form from catalog except apiKey", async () => {
    ...
    expect(screen.getByLabelText("Auto-archive interval (minutes)")).toHaveProperty(
      "value",
      "10080",
    );
    expect(screen.getByLabelText("Auto-delete interval (minutes)")).toHaveProperty(
      "value",
      "43200",
    );
  ...
  it("hides other catalog providers once one is configured", async () => {
    ...
    expect(
      screen.queryByText("One provider is set up. Update it or remove it to switch."),
    ).toBeNull();
  });
});
