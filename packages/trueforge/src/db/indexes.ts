...
/**
 * `(tenant_id, source.type, source.id)` on session — list filter by source.
 * Partial WHERE source IS NOT NULL (interactive sessions omit source).
 */
export const SESSION_SOURCE_IDX = 'session_source_idx';
