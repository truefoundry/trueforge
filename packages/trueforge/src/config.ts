...
export interface SharedServerConfiguration {
   ...
   * the server over HTTP. Env: `CONTROLLER_SERVER_BASE_URL`. Default: `http://localhost:$PORT`,
   ...
   */
  CONTROLLER_SERVER_BASE_URL: string;
  /**
...
const shared: SharedServerConfiguration = {
  ...
  CONTROLLER_SERVER_BASE_URL:
    getEnv('CONTROLLER_SERVER_BASE_URL', { defaultValue: `http://localhost:${String(port)}` }) ??
