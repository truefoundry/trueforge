...
export async function resolveSandboxProvider({
  ...
  if (record !== undefined) {
    // Reuse the image recorded when this provider was built, so a running-binary image bump
    // cannot redirect an existing tenant's sandboxes onto an unbuilt image.
