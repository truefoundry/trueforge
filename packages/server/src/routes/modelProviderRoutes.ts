/**
 * Model-provider route definitions (mounted at /api/v1/model-providers).
...
export const getModelProviderCatalogRoute = createRoute({
  ...
  description:
    ...
    'into PUT /model-providers to configure it. Custom providers are not listed here.',
  'x-fern-sdk-group-name': ['modelProviders'],
...
export const listModelProvidersRoute = createRoute({
  ...
  description: 'All configured providers with their models. API keys are redacted to `auth.api_key_set`.',
  'x-fern-sdk-group-name': ['modelProviders'],
...
export const putModelProviderRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      description: 'The saved provider, with `auth` redacted.',
    },
    400: {
