...
// the MCP spec owns the tool shape.
...
const McpAuthorizeQuerySchema = z.object({
  redirect_url: z.url().describe('Where to send the browser after OAuth completes.'),
});

const McpAuthorizeResponseSchema = z
  .object({
    status: z.enum(['authenticated', 'authentication_required']),
    auth_url: z.url().optional().describe('Present only when status is authentication_required.'),
  })
