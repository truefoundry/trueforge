...
export function createTurnsRouter(deps: TurnsRouterDeps) {
  ...
  const downloadTurnFileHandler: RouteHandler<typeof downloadTurnFileRoute> = async c => {
    const { session_id: sessionId, turn_id: turnId } = c.req.valid('param');
    ...
    try {
      ...
      if (!session.agent_spec.config.sandbox.file_downloads) {
      ...
      if (sandboxId === undefined) {
        return c.json({ error: { message: `Turn has no sandbox: ${turnId}` } }, 404);
      }
  ...
  router.openapi(downloadTurnFileRoute, downloadTurnFileHandler);
