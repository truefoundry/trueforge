...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS thread_capability_state`.execute(db);
  await sql`DROP TABLE IF EXISTS thread_context_log`.execute(db);
  await sql`DROP TABLE IF EXISTS session_event`.execute(db);
  await sql`DROP TABLE IF EXISTS turn_thread_context`.execute(db);
  await sql`DROP TABLE IF EXISTS turn_thread`.execute(db);
  await sql`DROP TABLE IF EXISTS turn`.execute(db);
