// This file was auto-generated by Fern from our API Definition.

import type * as TrueHarness from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";
import { CatalogProviderModelsItem } from "./CatalogProviderModelsItem.js";
import { CatalogProviderType } from "./CatalogProviderType.js";
import { ResourceName } from "./ResourceName.js";

export const CatalogProvider: core.serialization.ObjectSchema<
    serializers.CatalogProvider.Raw,
    TrueHarness.CatalogProvider
> = core.serialization.object({
    models: core.serialization.list(CatalogProviderModelsItem),
    name: ResourceName,
    type: CatalogProviderType,
});

export declare namespace CatalogProvider {
    export interface Raw {
        models: CatalogProviderModelsItem.Raw[];
        name: ResourceName.Raw;
        type: CatalogProviderType.Raw;
    }
}
