// This file was auto-generated by Fern from our API Definition.

import * as TrueHarnessTypes from "../../src/api/index";
import { TrueHarness } from "../../src/Client";
import { mockServerPool } from "../mock-server/MockServerPool";

describe("ModelProvidersClient", () => {
    test("list", async () => {
        const server = mockServerPool.createServer();
        const client = new TrueHarness({ maxRetries: 0, environment: server.baseUrl });

        const rawResponseBody = {
            data: [
                {
                    auth: { api_key_set: true },
                    base_url: "base_url",
                    models: [
                        { model_id: "model_id", name: "name", properties: { context_length: 1, max_output_tokens: 1 } },
                    ],
                    name: "name",
                    type: "openai",
                },
            ],
        };

        server
            .mockEndpoint()
            .get("/api/v1/model-providers")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.modelProviders.list();
        expect(response).toEqual({
            data: [
                {
                    auth: {
                        apiKeySet: true,
                    },
                    baseUrl: "base_url",
                    models: [
                        {
                            modelId: "model_id",
                            name: "name",
                            properties: {
                                contextLength: 1,
                                maxOutputTokens: 1,
                            },
                        },
                    ],
                    name: "name",
                    type: "openai",
                },
            ],
        });
    });

    test("upsert (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new TrueHarness({ maxRetries: 0, environment: server.baseUrl });
        const rawRequestBody = {
            auth: { api_key: "api_key" },
            base_url: "base_url",
            models: [{ model_id: "model_id", name: "name", properties: { context_length: 1, max_output_tokens: 1 } }],
            name: "name",
            type: "openai",
        };
        const rawResponseBody = {
            data: {
                auth: { api_key_set: true },
                base_url: "base_url",
                models: [
                    { model_id: "model_id", name: "name", properties: { context_length: 1, max_output_tokens: 1 } },
                ],
                name: "name",
                type: "openai",
            },
        };

        server
            .mockEndpoint()
            .put("/api/v1/model-providers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.modelProviders.upsert({
            auth: {
                apiKey: "api_key",
            },
            baseUrl: "base_url",
            models: [
                {
                    modelId: "model_id",
                    name: "name",
                    properties: {
                        contextLength: 1,
                        maxOutputTokens: 1,
                    },
                },
            ],
            name: "name",
            type: "openai",
        });
        expect(response).toEqual({
            data: {
                auth: {
                    apiKeySet: true,
                },
                baseUrl: "base_url",
                models: [
                    {
                        modelId: "model_id",
                        name: "name",
                        properties: {
                            contextLength: 1,
                            maxOutputTokens: 1,
                        },
                    },
                ],
                name: "name",
                type: "openai",
            },
        });
    });

    test("upsert (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new TrueHarness({ maxRetries: 0, environment: server.baseUrl });
        const rawRequestBody = {
            auth: { api_key: "x" },
            base_url: "base_url",
            models: [
                { model_id: "x", name: "xy", properties: { context_length: 1, max_output_tokens: 1 } },
                { model_id: "x", name: "xy", properties: { context_length: 1, max_output_tokens: 1 } },
            ],
            name: "xy",
            type: "openai",
        };
        const rawResponseBody = { error: { message: "message" } };

        server
            .mockEndpoint()
            .put("/api/v1/model-providers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.modelProviders.upsert({
                auth: {
                    apiKey: "x",
                },
                baseUrl: "base_url",
                models: [
                    {
                        modelId: "x",
                        name: "xy",
                        properties: {
                            contextLength: 1,
                            maxOutputTokens: 1,
                        },
                    },
                    {
                        modelId: "x",
                        name: "xy",
                        properties: {
                            contextLength: 1,
                            maxOutputTokens: 1,
                        },
                    },
                ],
                name: "xy",
                type: "openai",
            });
        }).rejects.toThrow(TrueHarnessTypes.BadRequestError);
    });

    test("catalog", async () => {
        const server = mockServerPool.createServer();
        const client = new TrueHarness({ maxRetries: 0, environment: server.baseUrl });

        const rawResponseBody = {
            data: [
                {
                    models: [
                        { model_id: "model_id", name: "name", properties: { context_length: 1, max_output_tokens: 1 } },
                    ],
                    name: "name",
                    type: "openai",
                },
            ],
        };

        server
            .mockEndpoint()
            .get("/api/v1/model-providers/catalog")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.modelProviders.catalog();
        expect(response).toEqual({
            data: [
                {
                    models: [
                        {
                            modelId: "model_id",
                            name: "name",
                            properties: {
                                contextLength: 1,
                                maxOutputTokens: 1,
                            },
                        },
                    ],
                    name: "name",
                    type: "openai",
                },
            ],
        });
    });
});
