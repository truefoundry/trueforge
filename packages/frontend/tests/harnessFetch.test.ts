...
describe('toHarnessUrl', () => {
  it('maps both session prefixes onto /api/v1/legacy/sessions', () => {
    assert.equal(toHarnessUrl('/v1/agents/draft-sessions'), '/api/v1/legacy/sessions');
    assert.equal(toHarnessUrl('/v1/agents/sessions'), '/api/v1/legacy/sessions');
    assert.equal(toHarnessUrl('/v1/agents/sessions/ses_1/turns'), '/api/v1/legacy/sessions/ses_1/turns');
    assert.equal(toHarnessUrl('/v1/agents/draft-sessions?limit=10'), '/api/v1/legacy/sessions?limit=10');
  });

  it('preserves the origin of absolute urls', () => {
    assert.equal(
      toHarnessUrl('http://localhost:8790/v1/agents/sessions/ses_1'),
      'http://localhost:8790/api/v1/legacy/sessions/ses_1',
    );
  });

  it('leaves other paths untouched', () => {
    assert.equal(toHarnessUrl('/api/v1/models'), '/api/v1/models');
    assert.equal(toHarnessUrl('/api/v1/legacy/sessions/ses_1'), '/api/v1/legacy/sessions/ses_1');
    ...
    assert.equal(
      toHarnessUrl('/api/v1/legacy/sessions?next=/v1/agents/sessions'),
      '/api/v1/legacy/sessions?next=/v1/agents/sessions',
    );
  });
});
...
describe('harnessFetch', () => {
  it('rewrites string, URL and Request inputs through the global fetch', async () => {
    ...
    assert.deepEqual(seen, [
      ...
      'http://localhost:8790/api/v1/legacy/sessions',
      'http://localhost:8790/api/v1/legacy/sessions/ses_1',
    ]);
  });
  ...
  it('passes a bodied Request through untouched when the path needs no rewrite', async () => {
    ...
    const request = new Request('http://localhost:8790/api/v1/legacy/sessions', {
      method: 'POST',
      body: '{"a":1}',
    });
    try {
