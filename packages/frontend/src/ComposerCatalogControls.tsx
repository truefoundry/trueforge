...
export function ComposerCatalogControls({
  ...
  if (mode === 'model') {
    return (
      <div ref={modelRootRef} style={{ position: 'relative' }}>
        <button
          type="button"
          className="chip-btn"
            ...
            <div className="model-menu">
              {models.map(model => (
                <button
                  key={model.name}
                  type="button"
                  data-active={model.name === selectedModel || undefined}
                  onClick={() => {
                    updateAgentSpec({ model: { name: model.name } });
                    setModelOpen(false);
                  }}
                >
  ...
  if (mode === 'skills-chip') {
    ...
    return (
      <button
        ...
        className="chip-btn chip-btn-outline"
        disabled={disabled}
        aria-disabled={!sandboxSelected || undefined}
        data-disabled={!sandboxSelected || undefined}
        title={!sandboxSelected ? 'Enable sandbox to use skills.' : undefined}
        onClick={() => {
          if (sandboxSelected) onOpenPanel?.('skills');
        }}
      >
        <span className="chip-doc-icon" aria-hidden />
        <span>{selectedCount > 0 ? `${String(selectedCount)} ` : ''}Skills</span>
      </button>
    );
  }
