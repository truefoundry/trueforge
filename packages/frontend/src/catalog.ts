...
/** Mirrors `McpServerAuthSettingsSchema` in packages/server/src/legacy-registry-store/schemas.ts. */
...
export async function listSkills(): Promise<Harness.SkillEntry[]> {
  const body = await fetchJson<ListEnvelope<Harness.SkillEntry>>('/api/v1/legacy/skills');
  return body.data;
}
