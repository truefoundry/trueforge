...
export function runStoreContractSuite(createStore: () => ISessionStore) {
  ...
  describe('session CRUD + activity', () => {
    it('createSession stores inline agent_spec and sets last_activity_timestamp_ms', async () => {
      ...
      expect(mustGet(session).agent_id).toBeNull();
      expect(mustGet(session).agent_spec).toEqual(
        expect.objectContaining({ model: { name: 'test-provider/test-model' } }),
      );
      expect(mustGet(session).last_activity_timestamp_ms).toBeGreaterThanOrEqual(before);
      expect(mustGet(session).title).toBeNull();
    });

    it('createSession accepts named agent_id XOR and listSessions filters by agent_id', async () => {
      const store = createStore();
      await store.createSession({
        tenant_id: tenant,
        session_id: 'named-1',
      ...
      expect(named.agent_id).toBe('agent-abc');
      expect(named.agent_spec).toBeNull();
      ...
      await expect(
        store.updateSession({
          tenant_id: tenant,
          session_id: 'named-1',
          agent_spec: makeAgentSpec({ instructions: 'nope' }),
          title: undefined,
        }),
      ).rejects.toBeInstanceOf(SessionStoreInvariantError);
    });
    ...
    it('updateSession patches only provided fields and bumps activity', async () => {
      ...
      expect(mustGet(after).agent_spec).toEqual(expect.objectContaining({ instructions: 'updated' }));
  ...
  describe('events + threads + capability_state', () => {
    ...
    it('JSON-looking strings remain strings', async () => {
      ...
      expect(session.agent_spec).toEqual(expect.objectContaining({ instructions: jsonLooking }));
  ...
  describe('deep-copy boundary', () => {
    it('mutating a returned session does not mutate the store', async () => {
      ...
      await seedSession(store);
      const session = mustGet(await store.getSession({ tenant_id: tenant, session_id: sessionId }));
      const spec = session.agent_spec;
      if (spec === null) {
        throw new Error('expected inline agent_spec');
      }
      spec.instructions = 'mutated';
      const again = mustGet(await store.getSession({ tenant_id: tenant, session_id: sessionId }));
      expect(again.agent_spec).toEqual(expect.objectContaining({ instructions: 'You are a test agent.' }));
    });
  });
