...
describe('AgentConfigEditors', () => {
  ...
  it.each(['__proto__', 'constructor', 'toString'])(
    'explicitly clears the prototype-named parameter %s when omitted from JSON',
    key => {
      const onChange = vi.fn();
      const params = Object.fromEntries([[key, 'initial']]);
      render(
        <SlotsProvider>
      ...
      fireEvent.click(screen.getByRole('button', { name: 'JSON' }));
      fireEvent.change(screen.getByRole('textbox', { name: 'JSON parameters editor' }), {
        target: { value: '{}' },
      });

      const changedParams = onChange.mock.lastCall?.[0].model.params;
      expect(Object.hasOwn(changedParams ?? {}, key)).toBe(true);
      expect(changedParams?.[key]).toBeUndefined();
    },
  );
  ...
  it('commits edits and removals while a blank parameter row is present', () => {
    const params = Object.fromEntries([['vendor_option', 'first']]);
    const spec: AgentSpec = {
      model: {
        name: 'openai/gpt',
        params,
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'Value for vendor_option' }), {
      target: { value: 'updated' },
    });
    expect(onChange).toHaveBeenLastCalledWith({
      ...spec,
      model: { ...spec.model, params: { vendor_option: 'updated' } },
    });

    fireEvent.click(screen.getByRole('button', { name: 'Remove vendor_option' }));
    expect(onChange).toHaveBeenLastCalledWith({
      ...spec,
      model: { ...spec.model, params: { vendor_option: undefined } },
    });
  });
