...
describe('useResourcePermissions', () => {
  it('allows without a permissions port and makes no request', () => {
    ...
    expect(result.current.allows('agent-1', 'DELETE')).toBe(true);
  });

  it('fails closed until grants load and denies missing ids', async () => {
    let resolvePermissions: ((value: { data: Record<string, ('MANAGE' | 'DELETE')[]> }) => void) | undefined;
    const listPermissions = vi.fn(
      () =>
        new Promise<{ data: Record<string, ('MANAGE' | 'DELETE')[]> }>(resolve => {
    ...
    await act(async () => {
      resolvePermissions?.({ data: { 'agent-1': ['MANAGE'] } });
    });
  ...
  it('ignores grants returned for a stale resource set', async () => {
    const resolvers = new Map<string, (value: { data: Record<string, ('MANAGE' | 'DELETE')[]> }) => void>();
    const listPermissions = vi.fn(
      ({ resourceIds }: { resourceIds: string[] }) =>
        new Promise<{ data: Record<string, ('MANAGE' | 'DELETE')[]> }>(resolve => {
          const resourceId = resourceIds[0];
          if (resourceId !== undefined) resolvers.set(resourceId, resolve);
        }),
    );
    const server = createMockAgentUIServer({ permissions: { listPermissions } });
    const wrapper = ({ children }: { children: ReactNode }) => (
    ...
    const { result, rerender } = renderHook(
      ({ resourceId }) => useResourcePermissions({ resourceType: 'agent', resourceIds: [resourceId] }),
      { initialProps: { resourceId: 'old' }, wrapper },
    );

    await waitFor(() => expect(resolvers.has('old')).toBe(true));
    rerender({ resourceId: 'current' });
    await waitFor(() => expect(resolvers.has('current')).toBe(true));
    await act(async () => {
      resolvers.get('current')?.({ data: { current: ['MANAGE'] } });
    });
    expect(result.current.allows('current', 'MANAGE')).toBe(true);

    await act(async () => {
      resolvers.get('old')?.({ data: { old: ['MANAGE', 'DELETE'] } });
    });
    expect(result.current.allows('current', 'MANAGE')).toBe(true);
    expect(result.current.allows('old', 'DELETE')).toBe(false);
  });
});
