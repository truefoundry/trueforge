...
Mutable composers expose **Agent Config** for live model parameters, instructions, runtime behavior, per-connector MCP tools, and skills. The compact Tools picker contains only Connectors and Skills. The Save Agent dialog only edits the agent name and description while preserving the active draft configuration; cancelling it discards those local edits. Model context and output limits render when the server supplies that optional catalog metadata.
