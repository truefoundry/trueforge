...
There are two architecture patterns for combining agents with sandboxes, based on where the agent runs.
...
```mermaid
flowchart LR
    App[Your application] <-->|"Network"| SB
    subgraph SB["Sandbox"]
...
The agent runs **outside** the sandbox — on your server. When it needs to execute code, it calls sandbox tools (`execute`, `read_file`, `write_file`) which run operations in the remote sandbox.
...
| Update agent logic without rebuilding images | |
| Sandbox failures don't lose agent state | |
| Pay only for execution time | |

## TrueForge's approach

TrueForge uses the **sandbox as tool** pattern: the agent and all orchestration logic run in the harness, and the sandbox is only used for code execution, file operations, and shell commands.
...
## Where the sandbox provider list comes from

The provider presets under **Settings → Sandbox providers** come from a **shipped catalog YAML**:

[`packages/server/src/catalog/sandbox-catalog.yaml`](https://github.com/truefoundry/trueforge/blob/main/packages/server/src/catalog/sandbox-catalog.yaml)

Today that catalog includes a [Daytona](https://www.daytona.io) preset. The server exposes it at `GET /api/v1/settings/sandbox-providers/catalog`. Copy a preset (or configure via the UI), fill in your API key, and TrueForge can provision sandboxes on demand. Override the shipped list with `SANDBOX_CATALOG_PATH` if you want custom presets. Whether a provider is configured is reported by `GET /api/v1/capabilities`.

## Configuring a sandbox provider

Configure the provider once — in the UI under **Settings → Sandbox providers** or via `PUT /api/v1/settings/sandbox-providers`:
