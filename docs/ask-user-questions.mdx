...
Agents often hit decision points where they cannot make a safe assumption — which environment to deploy to, which of three matching resources the user meant, what an ambiguous required field should be set to. Without a structured ask, the agent either guesses or dumps a free-form question the UI cannot treat as a first-class action.

**Ask User Questions** pauses the TrueForge agent, shows a multiple-choice prompt in chat (or your client), and resumes with the chosen answer. It is **enabled by default** on every agent.
