...
## Set it in Build Agent

Each connector's chip in Build Agent's **MCP Servers** section has a book-icon **preload toggle** — off by default. Hover it to see the tradeoff, click to switch it on.

<Columns cols={2}>
  <Frame caption="Off (default) — tools are discovered on demand.">
    <img
      src="/images/preload-tools-off.png"
      alt="A connector chip in Build Agent with its preload toggle off; the tooltip reads Preload tools OFF — the agent discovers tools on demand, lighter context upfront"
    />
  </Frame>
  <Frame caption="On — tool definitions load into context upfront.">
    <img
      src="/images/preload-tools-on.png"
      alt="A connector chip in Build Agent with its preload toggle on and highlighted; the tooltip reads Preload tools ON — tool definitions load into context upfront"
    />
  </Frame>
</Columns>
