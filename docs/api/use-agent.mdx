...
Keep an **`id`-keyed event index**. Model output streams as an empty `model.message` base followed by `model.message.delta` fragments that share the base's `id`; store each non-delta event under its `id`, and merge each delta into the base with `isEventDelta` / `mergeEventDelta` (TypeScript). The Python SDK has no merge helpers — skip events whose `type` ends in `.delta`. (Deltas appear only while streaming; [`listTurnEvents`](#replay-a-finished-turn) / `list_turn_events` returns them already merged.) The pause handlers below reuse this index to look up the `model.message` that emitted a tool call.
...
A gated tool ([human approval](/create-agent/overview#whats-in-an-agent)) emits [`tool.approval_required`](#tool-approval-required). Collect the pending events while streaming, then resume with one `user.tool_approval` per pending call, either allowing it or denying it with a reason. TypeScript merges deltas into the event index while streaming. Python has no merge helpers, so after the stream closes, rebuild the index from [`listTurnEvents`](#replay-a-finished-turn) / `list_turn_events` (already merged) before looking up `sourceEventId` / `source_event_id`.
...
Keep a **per-thread** index (`Map<threadId, Map<id, event>>` in TypeScript; nested dicts in Python) and merge deltas within the matching thread's bucket. Python skips `.delta` events instead of merging.
