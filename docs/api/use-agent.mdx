...
For the conceptual hierarchy (Agent → Session → Turn → Event → Delta) and a short end-to-end walkthrough, see [Using the API](/api/overview). For every event field, see [Turn events](/api/turn-events).
...
const client = new TrueForge({
  ...
  // Optional: pass whatever your deployment uses for auth.
  // headers: { Authorization: `Bearer ${process.env.TRUEFORGE_TOKEN}` },
});
```

The harness has no built-in API-key auth. Point `baseUrl` at your deployment (standalone default: `http://localhost:8790`). Pass `headers` when your reverse proxy or OIDC layer requires them.
...
A single turn stream interleaves events from the root agent and any [subagents](/context-engineering/subagents) that run in parallel. Every event carries a `threadId`:
