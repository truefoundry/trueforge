...
### Update an inline spec

Only **inline** sessions can change their agent. `sessions.update` replaces `agent` with a new `{ spec }`. Named (reference) sessions reject this with `422`. An empty body is a no-op that refreshes `updatedAt`.
...
const { data: updated } = await client.sessions.update(session.id, {
  agent: {
    spec: {
      model: { name: 'anthropic/claude-sonnet-4-6' },
      instructions: 'You are a terse research assistant. Prefer bullet lists.',
    },
  },
});
console.log(updated.agent.type); // "inline"
```

Later turns on this session use the new spec. History from earlier turns is unchanged.
...
if (turnId == null) throw new Error('turn.created was never received');
...
| `created_at` | ISO 8601 timestamp. Optional on `model.message.delta`; required on every other event.                                                                                                                              |
...
- **`cancelled`** — carries `reason` (`client-cancelled`, `server-execution-timeout`, `cancelled-for-next-turn`, or `abandoned`), `completed_at`, and optional `metrics`.
- **`error`** — carries `message`, `completed_at`, and optional `metrics`.
...
| Field           | Description                                                                                                                                                                                    |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content`       | Assistant text, or an array of `{ type: "text", text }` / `{ type: "refusal", refusal }` parts. Empty or omitted when the message is only tool calls.                                           |
| `tool_calls`    | Tool invocations. Each has `id`, `type: "function"`, `function` (`name`, `arguments`), and `tool_info` (`mcp` or `truefoundry-system`). |
...
| `agent_info` | `{ type: "dynamic", name, input, model? }` — generated instructions, plus an optional model override. |
