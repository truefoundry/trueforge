...
That `agent` is a **reference** to the named registry agent. A session can instead hold an [inline spec](/create-agent/overview#create-an-agent-via-the-api) (`type: "inline"`). Persist the session `id` so Jane can come back tomorrow and pick up where she left off. Only one turn runs in a session at a time.
