...
A tool call needs [human approval](/create-agent/tool-approval) before it can run. The turn ends after this event; resume with a `user.tool_approval` input.
...
A client-side tool (e.g. [`ask_user_question`](/create-agent/ask-user-questions)) needs a result from your app. Same shape as `tool.approval_required`; resume with a `user.tool_response` input.
