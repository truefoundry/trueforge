...
While a turn runs, the server streams events as Server-Sent Events (SSE). Each SSE message's `data` is one JSON event; the SSE `id` field carries the per-stream **sequence number**, which you can pass back (as a `Last-Event-Id` header or the `after_sequence_number` query parameter) to the [subscribe endpoint](/api/overview#non-streaming-reconnects-and-cancellation) to resume after a disconnect.
...
| `thread_id`  | Which execution thread emitted it: `"main"` for the root agent, a generated id for [subagents](/context-engineering/subagents), or `null` for run-level events (`turn.*`, `sandbox.created`, `mcp.auth_required`). |

A stream always opens with `turn.created` and closes with `turn.done`. When you list persisted events afterwards (`GET .../events`), you get the same events with deltas pre-merged into their `model.message`.
...
A tool call needs [human approval](/human-in-the-loop) before it can run. The turn ends after this event; resume with a `user.tool_approval` input.
...
A client-side tool (e.g. [`ask_user_question`](/ask-user-questions)) needs a result from your app. Same shape as `tool.approval_required`; resume with a `user.tool_response` input.

## Subagent threads

### `thread.created`

A [subagent](/context-engineering/subagents) started. Subsequent events with this `thread_id` belong to it.
