  ...
  <Expandable title="ThemeConfig">
    ...
    <ParamField path="brand" type="BrandConfig">
      <Expandable title="BrandConfig">
        <ParamField path="logo" type="BrandImage">
          Falls back to the default mark plus the brand name.
        </ParamField>
        <ParamField path="icon" type="BrandImage">
          Falls back to the bare mark — no name.
        </ParamField>
        <ParamField path="name" type="string" default='"TrueFoundry"'>
          Shown beside the default logo.
        </ParamField>
      </Expandable>

      `BrandImage` accepts `{ src, alt? }`, a React node, or a render function.
    </ParamField>
...
<ParamField path="overrides" type="SlotOverrides">
  ...
  table — so editor autocomplete lists exactly the 59 overridable names. See
...
<TrueforgeUI
  ...
  theme={{ preset: "claude", brand: { name: "Acme" } }}
