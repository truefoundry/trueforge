...
| `BrandLogo` | Renders `theme.brand.logo`; falls back to a default mark plus brand name. Props: `{ className? }`. |
| `BrandIcon` | Renders `theme.brand.icon`, falling back to the bare default mark — no name. Props: `{ className? }`. |
...
`ThemeConfig`, `ThemeMode`, `ThemePreset`, `SemanticTokens`, `BrandConfig`, `BrandImage`,
