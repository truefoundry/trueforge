...
Chat and agent-builder operations are required. Settings, agent details, schedules, metrics, and permissions are optional plugins.

When `permissions` is omitted, the UI does not request grants and actions keep
their normal enabled state. A configured permissions server is fail-closed:
MANAGE/DELETE actions remain visible but disabled while loading, when denied,
or when the request fails.

```ts
interface PermissionsServer {
  listPermissions(req: {
    resourceType: "agent" | "schedule" | "session";
    resourceIds: string[];
  }): Promise<{
    data: Record<string, ("MANAGE" | "DELETE")[]>;
  }>;
}
```
