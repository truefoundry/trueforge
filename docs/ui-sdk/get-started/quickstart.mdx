...
Two notes on this path. Empty `getModels` means the composer has no models to offer, so supply
real ones before enabling composer mode — see [Agent modes](/ui-sdk/concepts/agent-modes). And
