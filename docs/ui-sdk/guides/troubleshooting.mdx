...
<AccordionGroup>
  ...
  <Accordion title="An error alert appears immediately on mount">
    If you passed `type: "trueforge"`, that is expected — the harness adapter has not shipped, so it resolves to a not-implemented error. Use `type: "truefoundry"` or a [custom server](/ui-sdk/guides/custom-server) instead.

    Otherwise the server failed to resolve: check that `apiKey` and `controlPlaneURL` are set and reachable. Attach an `onError` handler to see the underlying error.
  </Accordion>
