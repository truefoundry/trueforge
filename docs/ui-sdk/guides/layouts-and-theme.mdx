...
<Tabs>
  ...
  <Tab title="drawer">
    Chat history sits in an overlay drawer opened from the history icon. Useful on smaller viewports where a persistent sidebar would crowd the chat.
...
Built-in layouts are code-split: only the layout you pick lands in the bundle. A lightweight placeholder shows while it loads.
