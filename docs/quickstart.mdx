...
<Steps>
  ...
  <Step title="Add a sandbox provider">
    A [sandbox](/sandbox) lets the agent run code, work with files, and use skills — so it's required for the skill you just enabled. The Settings UI supports [Daytona](https://www.daytona.io); hosted deployments can also configure Kubernetes through environment variables. For this walkthrough, use Daytona:
  ...
  <Step title="Build the agent">
    Open **Build Agent** from the sidebar. It's a split view: assemble the agent in the **Agent Config** panel on the left, and test it in a live chat on the right as you go.

    In the left panel, set:

    - **Model** — pick one from the selector at the top and choose a reasoning effort.
    - **Instructions** — define the agent's role and behavior. For this example:
    ...
    - **MCP Servers** — open **Select MCP Tools**, choose **Exa**, and enable its tools.
    - **Skills** — open the **Skills** picker and enable **web-artifacts-builder**.
    - **Runtime Config** — open it to review execution and context settings. Keep **Dynamic sub-agents** on so the agent researches each item in parallel, and leave **Generative UI**, **Sandbox**, and **Context compaction** on.

    <Tabs>
      <Tab title="Tools">
        <Frame>
          ![The Select MCP Tools dialog with Exa selected and its web_search and web_fetch tools enabled](/images/quickstart-build-tools.png)
        </Frame>
      </Tab>
      <Tab title="Skills">
        <Frame>
          ![The Skills dialog with web-artifacts-builder enabled](/images/quickstart-build-skills.png)
        </Frame>
      </Tab>
      <Tab title="Runtime Config">
        <Frame>
          ![The Runtime Config panel with Dynamic sub-agents, Generative UI, Ask user questions, Sandbox, and Context compaction toggled on](/images/quickstart-build-runtime.png)
        </Frame>
      </Tab>
    </Tabs>

    Then test it in the chat on the right. For example:
    ...
    <Frame caption="The Build Agent page: Agent Config on the left, and on the right the interactive brief the agent produced, including a Generative UI radar chart of the databases' relative strengths.">
      ![The Build Agent page with the Agent Config panel on the left (model, instructions, runtime config, MCP servers, and skills) and, on the right, an interactive brief comparing Qdrant, Weaviate, and Milvus with a radar chart of relative strengths and recommendation cards](/images/quickstart-build-agent.png)
    </Frame>
  </Step>

  <Step title="Save it as an agent">
    Happy with the setup? Click **Save Agent** at the top right, give it a name and a short description, and click **Save changes**. This captures the model, instructions, tools, skills, and runtime settings together as a reusable agent. For this example:

    - **Agent name** — `web-research-brief`
    - **Description** — `Web research brief agent`

    <Frame>
      ![The Save agent dialog with an agent name and a description field](/images/save-agent.png)
    </Frame>
  </Step>

  <Step title="Find it under Agents">
    Open **Agents** from the sidebar. Your saved agent is listed with a summary of its model, skills, and connectors. Click **Try** to start a fresh chat with the same setup, or use the row's menu to manage it. See [Agents](/agent-library) for shared-visibility notes in hosted mode.

    <Frame>
      ![The Agents list showing the saved web-research-brief agent with its model and tool summary and a Try action](/images/quickstart-agents-library.png)
    </Frame>
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Sessions" icon="magnifying-glass" href="/sessions">
    Inspect the run you just made: turns, tool calls, subagents, tokens, and timing.
  </Card>
  <Card title="Schedules" icon="clock" href="/schedules">
    Run a saved agent on a recurring cadence, unattended.
  </Card>
