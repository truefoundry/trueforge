...
<Tabs>
  <Tab title="Local mode (npx)">
    ...
    ```bash
    PUBLIC_BASE_URL=http://localhost:8790 npx @truefoundry/trueforge
    ```
  ...
  <Tab title="Hosted mode (Kubernetes)">
    The Helm chart deploys the server in hosted mode with bundled Postgres and Redis (or point it at your own). Set `server.publicBaseUrl` to the public origin:
...
<AccordionGroup>
  ...
  <Accordion title="What is PUBLIC_BASE_URL and when do I need to set it?">
    Required. The public origin the server hands to IdPs and MCP servers for OAuth callbacks.

    For local `npx` / standalone, set it to the origin you use in the browser:
