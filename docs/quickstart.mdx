...
<Tabs>
  ...
  <Tab title="Hosted mode (Kubernetes)">
    ...
    | `configs.oidc.enabled` | `false` | Enable IdP login via `configs.oidc.*` (see chart README).                |
...
<Steps>
  ...
  <Step title="Add a sandbox provider (optional)">
    ...
    2. **Add the provider** under **Settings → Sandbox providers** — choose Daytona and enter your API key.
  ...
  <Step title="Compose an agent and start chatting">
    ...
    ```text
    Use DeepWiki to explain the truefoundry/models repository — what it does, its architecture, and how to get started. Then use the pdf skill to save a one-page brief as a PDF.
    ```
