...
description: 'How TrueForge manages, optimizes, and constrains agent context within model limits.'
---

## What is context engineering?

Context engineering is the practice of providing the **right information in the right format** so an agent can reliably accomplish tasks. In TrueForge, _context_ is everything the model sees on each step — from system instructions and skills to tool definitions, conversation history, and tool results.

Getting context right has a direct impact on agent quality:

- **Too little context** — the agent lacks the information it needs and produces wrong or incomplete answers.
...
TrueForge automates the balance through a layered set of strategies. To understand them, it helps to think of context in two phases:
...
<Accordion title="Skills" icon="book">
  ...
  <Accordion title="MCP tool definitions" icon="plug">
    ...
    By default (`preload: false`), each attached MCP server contributes only its `name` and `description`; individual tool schemas are discovered on demand. Turn `preload` on only for small, frequently-used servers. See [Deferred tool loading](/context-engineering/deferred-tool-loading).
...
| **Subagent outputs**   | Final results returned from delegated [subagents](/context-engineering/subagents) |
...
<CardGroup cols={2}>
  <Card title="Subagents" icon="diagram-project" href="/context-engineering/subagents">
  ...
  <Card title="Large Result Offloading" icon="file-export" href="/context-engineering/large-tool-responses">
  ...
  <Card title="Code Mode" icon="code" href="/context-engineering/code-mode">
...
1. **Start with input context discipline.** Keep instructions focused on the agent's role; move procedures into skills; leave preload off for large MCP servers.
2. **Let progressive disclosure do the work.** Default to deferred loading for both skills and MCP servers. Override only when a server is truly needed on every run.
3. **Delegate heavy work to subagents.** Multi-step research or tasks with large intermediate outputs belong in subagents.
4. **Use Code Mode for aggregation.** When the model needs to count, group, filter, or join tool output, Code Mode keeps raw data out of context entirely and avoids arithmetic hallucinations.
5. **Watch token usage.** Every `model.message` event carries `usage` with an `input_tokens_breakdown` (harness, skills, instructions, tool definitions, messages), so you can see exactly where context is going.
