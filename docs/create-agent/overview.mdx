...
<AccordionGroup>
  ...
  <Accordion title="Sandbox" icon="box">
    ...
    Optional extras (API): allow clients to **download files** the agent produces, and **inject git credentials** so the sandbox can clone private repos without the agent ever seeing the secret.

    **Set in:** UI (on/off) · API (on/off + file downloads + git auth injection)
  </Accordion>
...
The [SDK](/api/overview) and HTTP API expose **every** option through the **agent spec**. You either save the spec as a named, reusable agent and reference it by name, or pass it inline when you open a session. Every field except `model` has a default.

JSON examples below use the HTTP wire (`snake_case`). TypeScript SDK snippets use camelCase (`mcpServers` ↔ `mcp_servers`, `requireApprovalForTools` ↔ `require_approval_for_tools`, `iterationLimit` ↔ `iteration_limit`).
...
**Update an inline spec** later with `sessions.update`. Only inline sessions accept this; named (reference) sessions reject it with `422`. See [Update an inline spec](/api/use-agent#update-an-inline-spec).
...
await client.agents.update(agent.id, {
  manifest: {
    model: { name: 'anthropic/claude-sonnet-4-6' },
    instructions: 'You help customers with orders. Always confirm before processing refunds.',
    mcpServers: [{ name: 'orders-api', requireApprovalForTools: ['process_refund'] }],
  },
});
```
  ...
  mcpServers: [
    {
      name: 'orders-api',
      enableTools: ['@all'],
      requireApprovalForTools: ['process_refund'],
      preload: false,
    },
  ],
  skills: [{ name: 'customer-support' }],
...
await client.agents.create({ name: 'support-bot', manifest: spec });
// or: await client.sessions.create({ agent: { spec } });
```
</CodeGroup>
