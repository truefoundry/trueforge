...
Answers are graded by an independent LLM judge that sees only the task's criteria and the answer, never which harness produced it. A task passes only if it meets **every** required criterion.
