...
Plain text and markdown work for short answers, but they fall flat when an agent needs a dashboard, a metrics comparison, a sortable table, or a short form. Users end up squinting at walls of numbers when a chart would have told the story in seconds.

**Generative UI** lets a TrueForge agent respond with interactive components — charts, tables, cards, forms, layouts — that the client renders as React UI. The agent decides _what_ to show; the harness streams it in a standard format. It is **enabled by default**, and the bundled chat UI renders it out of the box.
...
<Frame caption="The chat UI renders the OpenUI snippet as a live dashboard — summary cards, a bar chart, and a table.">
  <img src="/images/generative-ui.png" alt="A rendered Generative UI dashboard with summary cards, a grouped bar chart, and a table" />
</Frame>
