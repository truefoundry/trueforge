...
| `Your email is not allowed…` / `email_not_allowed` | Caller’s `email` claim is outside `OIDC_ALLOWED_EMAILS`, or the ID token has no `email` claim while an allowlist is set |
