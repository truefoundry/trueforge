...
title: 'Okta'
sidebarTitle: 'Okta'
description: 'Step-by-step: configure Okta as the OIDC identity provider for TrueForge.'
---

Use this after reading the [Setup Login overview](/authentication/overview). Below is Okta-specific setup only.
...
Helm: set `server.publicBaseUrl` and `configs.oidc`. Prefer `clientSecret` via
`valueFrom.secretKeyRef` to a Secret you manage:
