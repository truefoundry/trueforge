...
Chart `version` / `appVersion` / `image.tag` are written on `main` by the
image+Helm release workflow (or a human). Publishing is gated by git tag
`charts/trueforge@<version>`.
