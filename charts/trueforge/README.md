...
**external** Redis, set `redis.enabled=false` and `externalRedis.enabled=true` with `url` and/or `host` (or Sentinel)
as a string or `valueFrom.secretKeyRef`. When both are set, the app prefers `REDIS_URL`:
